...
parser.add_argument('--type', type=str, default='all', choices=['all', 'isr-pal', 'refugee', 'tr-gr'], help='Data type to train on')
parser.add_argument('--subtask', type=str, help='Subtask to test', choices=['1', '2', '3', '4'])
...
if args.subtask is None:
    ...
    test_texts = [el['text'] for el in test_data_l]
    print('Test data loaded')
    print('Test data length: {}'.format(len(test_texts)))
...
max_features = 3000
vectorizer1 = TfidfVectorizer(analyzer='char', ngram_range=(3, 3), max_features=max_features, lowercase=True, strip_accents='unicode')
vectorizer2 = TfidfVectorizer(max_features=max_features, lowercase=True, strip_accents='unicode')
...
if test:
    X_test = vectorizer1.transform(test_texts)
    X_test_t = vectorizer2.transform(test_texts)
